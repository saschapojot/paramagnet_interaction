...
#check stats of computed MC results of all .pkl files within a directory
...
for ldInd in range(0,len(pklFileNames)):
    print("processing file "+str(ldInd))
    ...
    xsTicks = [R, R + len(cumusAvg) * 1 / 4, R + len(cumusAvg) * 2 / 4, R + len(cumusAvg) * 3 / 4,
               R + len(cumusAvg) * 4 / 4]
